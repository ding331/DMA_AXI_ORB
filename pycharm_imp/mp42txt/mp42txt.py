...
video_start = 50
video_end = 400
vidcap = cv2.VideoCapture('highway_mountain.mp4')
...
def getFrame_BIN8bits(sec, bin_file):
    ...
    if hasFrames:
        ...
        print(f"Grayscale Pixel Values for Frame {count}:\n", gray)
...
def rot_BIN8bits(rot_bin_file):
    ...
    if hasFrames:
        ...
        # Print the rotated grayscale pixel values
        print(f"Rotated Grayscale Pixel Values for Frame {count}:\n", rot_gray)

        # Convert to a numpy array for easier manipulation
        rot_gray_np = np.array(rot_gray)

        # Save the rotated grayscale pixel values as a binary file
        with open(rot_bin_file, 'ab') as file:
            file.write(rot_gray_np.tobytes())  # Write the pixel values as binary data

    return rot_gray

bin_file = 'D:\\D_region\\GSlab_git_NAS\\HW10_SD_DDR_ILA_IRQ\\road_bin\\road720.bin'
...
while success:
    ...
    if video_end > count > video_start:
        ...
        rot_gray = rot_BIN8bits(rot_bin_file)
        cv2.imshow('G_slice', image)
        cv2.imwrite(f'gslice{count}.bmp', image)
        cv2.imwrite(f'slice_rot{count}.bmp', rot_gray)
